...
gpu_image = (
    ...
    .add_local_python_source("config", "train_ddp")
)
...
@app.function(
    ...
    timeout=60 * 60 * 6,  # 6-hour hard ceiling; budget cap is ~$40
)
def pretrain_run(resume: bool = False) -> None:
    import subprocess
    ...
    cmd = [
        ...
        "--standalone",
        "--nnodes=1",
        f"--nproc_per_node={config.PRETRAIN_GPU_COUNT}",
        "-m", "train_ddp",
    ]
    if resume:
        cmd.append("--resume")
...
@app.local_entrypoint()
def pretrain(resume: bool = False):
    print(f"Launching pretrain: {config.PRETRAIN_GPU_COUNT}x {config.PRETRAIN_GPU}  "
          f"budget_cap=${config.BUDGET_CAP_USD}  resume={resume}")
    # .spawn() submits the job and returns immediately — the cloud function runs
    # fully independently of this local process. Killing/closing this terminal
    # cannot cancel the training job. Use `.remote()` only if you need the result
    # synchronously; for long-running GPU jobs, always use .spawn().
    handle = pretrain_run.spawn(resume=resume)
    print(f"Job submitted and detached. Function ID: {handle.object_id}")
    print("Training runs independently — safe to close this terminal.")
