"""Single source of truth for the from-scratch 125M SLM build (slm125mLIVE-anand)."""
...
class ModelConfig:
    ...
    def flops_per_token(self) -> int:
        """6N (matmuls, fwd+bwd) + attention score/AV term. Used for MFU."""
        attn = 12 * self.num_hidden_layers * self.max_position_embeddings * self.hidden_size
        return 6 * self.approx_params() + attn
...
class TrainConfig:
    seq_len: int = SEQ_LEN
    micro_batch_size: int = 64          # chosen by the Phase 5a benchmark (40.2% MFU)
    global_batch_tokens: int = 524_288
    epochs: int = 4                     # 4 passes over ~2.19B unique -> ~8.8B seen
    lr: float = 6e-4
    min_lr: float = 6e-5
...
BUDGET_CAP_USD = 40.0
GPU_USD_PER_HOUR = 3.9492   # H100 SXM5 on Modal: $0.001097/s
...
if __name__ == "__main__":
    ...
    print(f"hf repo: {HF_REPO} | volume: {VOLUME_NAME}")
